...
class RisingCubsETLJob:
    ...
    def unit_test(self, dataframe, app_config):
        try:
            expected_data_path = "s3://rising-cubs-landing-bucket/dataset/sample_dataset/" + app_config["transformation-dataset"]["unit_datasets"][0]
            print(expected_data_path)
            df = spark.read.format("csv").option("header", "true").load(expected_data_path)
            # Converting the data to pandas
            df = df.toPandas()
            dataframe = dataframe.toPandas()
        except:
            print("Error while reading the file from s3 bucket")
        print("This is my dataframe",dataframe.exceptAll(df))
        if dataframe.exceptAll(df).show() == None:
...
def main(spark):
    ...
    Unit Testing
    """
    rising_cubs_etl_obj.unit_test(dataframe, app_config)
...
if __name__ == "__main__":
    ...
    app_config = (
        ...
        .json("s3://rising-cubs-landing-bucket/config_file/app_config_unit_test.json")
        .toJSON()
        .collect()
    )
